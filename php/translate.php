...
$fileName='';
if($argc < 2){
    exit('need command line param');
}else{
    $fileName=$argv[1];
}

$reader=Reader::createFromPath(__DIR__.'/storage/'.$fileName.'.csv','r+');
...
foreach ($records as $record) {
    ...
    if (array_key_exists('error_code', $decode)) {
        //出错
        var_dump('baidu translate api fail',$decode['error_code']);
    }else{
        $data[]=[$decode['trans_result'][0]['src'],$decode['trans_result'][0]['dst']];
    }
}
$writer->insertAll($data);
var_dump('execute script done');
